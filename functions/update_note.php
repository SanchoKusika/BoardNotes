...
if ($board->session_id) {
    ...
    if (!$isParticipant && !$isOwner) {
        echo json_encode(['success' => false, 'message' => 'No permission to update note in session board.']);
        exit();
    }
} else {
    if ($board->user_id != $userId) {
        echo json_encode(['success' => false, 'message' => 'No permission to update note in personal board.']);
        exit();
    }
}
